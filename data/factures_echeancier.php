...
if ($_GET['dateType'] == 'dateEcheance') $dateType = 'DATE_ECHEANCE';
else $dateType = 'DATE_FACTURE';
$codeType = '';
$statut = '';
$modePaiement = '';
$date_debut = '';
...
(isset($_GET['date_debut']) && $_GET['date_debut']<>'')?$date_debut=$_GET['date_debut']:$date_debut=date("Y") . '-'.  date("m"). '-' . '01';
(isset($_GET['date_fin']) && $_GET['date_fin']<>'')?$date_fin = $_GET['date_fin']:$date_fin = date("Y") . '-' . date("m") . '-' . date("t");

if(!isset($_GET['sort']) || is_null($_GET['sort'])) $orderBy = 'FACTURES.' . $dateType . ', NOM_FOURNISSEUR';
else if ($_GET['sort'] == 'NOM_FOURNISSEUR') 	$orderBy = "NOM_FOURNISSEUR, " . $dateType;
else if ($_GET['sort'] == 'DATE_ECHEANCE') $orderBy = "DATE_ECHEANCE, NOM_FOURNISSEUR ";
else if ($_GET['sort'] == 'DATE_FACTURE') $orderBy = "DATE_FACTURE, NOM_FOURNISSEUR ";
else if ($_GET['sort'] == 'CODE_TYPE') $orderBy = "CODE_TYPE, NOM_FOURNISSEUR ";
else if ($_GET['sort'] == 'MONTANT_TTC') $orderBy = "MONTANT_TTC, NOM_FOURNISSEUR ";
else if ($_GET['sort'] == 'CODE_STATUT') $orderBy = "CODE_STATUT, NOM_FOURNISSEUR ";
else if ($_GET['sort'] == 'CODE_MODE_PAIEMENT') $orderBy = "CODE_MODE_PAIEMENT, NOM_FOURNISSEUR ";

$whereClause = ' AND ' . $dateType . ' >= STR_TO_DATE("' . $date_debut . '", "%Y-%m-%d") AND ' . $dateType . ' <= STR_TO_DATE("' . $date_fin . '", "%Y-%m-%d")' ;

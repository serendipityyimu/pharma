...
$query = "SELECT ID_FOURNISSEUR, NOM_FOURNISSEUR, ECHEANCE, MODE_REGLEMENT FROM FOURNISSEURS  WHERE NOM_FOURNISSEUR LIKE '%". $_GET['s'] . "%' ORDER BY NOM_FOURNISSEUR ASC";
	...
	for ($i = 0 ; $i < $dataLen && count($results) < 10 ; $i++) {
	  ...
	  $results[] = array('id' => $result[$i]['ID_FOURNISSEUR'], 'name' => $result[$i]['NOM_FOURNISSEUR'], 'echeance' => $result[$i]['ECHEANCE'], 'mode_reglement' => $result[$i]['MODE_REGLEMENT']); // On ajoute alors le résultat à la liste à retourner
